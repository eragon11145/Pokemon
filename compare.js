...
var x, y, pokemon, pokemoN, pokemoN2, hp, atk, df, spa, sdf, spd, p, i,gen, poke,hp1,atk1,df1,spa1,sdf1,spd1,gen2,poke2,statS,staTs,z,score,roundActive;
roundActive=false
x=(Math.random())
console.log(x)
y=Math.random()
...
function newgame(){
  ...
  y=Math.floor(Math.random()*958)
  document.getElementById("Question").innerHTML='Which Pokemon has higher ' + staTs[z]+'?'
  document.getElementById("option1").innerHTML=pokemoN[x]
  document.getElementById("option2").innerHTML=pokemoN[y]
  hp1=(hp[x]);
  atk1=(atk[x]);
  df1=(df[x]);
  spa1=(spa[x]);
  ...
  if (document.getElementById("Generations").value !="all"){
    while (gen2!=parseInt(document.getElementById("Generations").value)){
      y=Math.floor(Math.random()*934)
      hp1=(hp[y]);
  atk1=(atk[y]);
  df1=(df[y]);
  spa1=(spa[y]);
  sdf1=(sdf[y]);
  spd1=(spd[y]);
  console.log(hp[y]+atk[y]+df[y]+spa[y]+sdf[y]+spd[y]);
  choosegen();
    }
  }
  if (document.getElementById("goodness").checked){
...
function guess1(){
  if (statS[z][x]>statS[z][y]){
    document.getElementById("feedback").innerHTML='Correct! Next Question'
        newgame();
    score +=1
